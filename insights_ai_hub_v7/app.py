...
from models import init_db, fetch_recent, fetch_analytics, fetch_last_analysis, fetch_latest_for_contact, log_analysis
from scheduler import start_scheduler, compute_insights_score, update_fub_insights_score, score_lead, detect_borough, gpt_analyze, build_prompt, TONE_PROFILES
from scheduler import fub_get, get_notes, get_emails, get_revaluate_score
...
def require_auth(request: Request):
    if request.url.path.startswith("/widget") or request.url.path.startswith("/api/"):
        return
...
def export_csv():
    ...
    c.execute("SELECT contact_name,score,category,tone,tone_mode,next_action,draft,insights_score,revaluate_score,created_at FROM analyses ORDER BY created_at DESC")
    data = c.fetchall(); conn.close()
    output = io.StringIO(); import csv as _csv
    ...
    output.seek(0)
    return StreamingResponse(output, media_type="text/csv", headers={"Content-Disposition": "attachment; filename=insights_ai_all_analyses.csv"})
...
def widget(request: Request, contact_id: int, include_market: int = 1, include_news: int = 1):
    ...
    if not last:
        return templates.TemplateResponse("widget.html", {"request": request, "data": None, "spark": [], "news": news, "market": market, "include_market":include_market, "include_news":include_news})
    spark = fetch_latest_for_contact(contact_id, limit=5)
    data = {"contact_id":contact_id,"name":last[0],"stage":last[1],"tags":last[2],"lead_score":last[3],"category":last[4],"tone":last[5],"tone_mode":last[6],"next_action":last[7],"draft":last[8],"insights_score":last[9],"reval":last[10],"created_at":last[11]}
    return templates.TemplateResponse("widget.html", {"request": request, "data": data, "spark": spark, "news": news, "market": market, "include_market":include_market, "include_news":include_news})
...
async def api_regen_draft(contact_id: int, tone: str = "Recommended", include_news: int = 1, include_market: int = 1):
    ...
    notes = get_notes(contact_id); emails = get_emails(contact_id)
    ...
    tone_txt, action, draft, _raw = gpt_analyze(person, notes, emails, lead, tone_mode=tone_mode, market_bullets=market.get("bullets"), news_items=news, borough_hint=borough_hint)
    ...
    log_analysis(contact_id, person.get("name"), person.get("stage"), person.get("tags"), lead, "WARM" if lead>=5 else "COOL", tone_txt, tone_mode, action, draft, insight, get_revaluate_score(person))
    return JSONResponse({"tone": tone_txt, "tone_mode": tone_mode, "next_action": action, "draft": draft, "insights_score": insight})

@app.post("/api/refine_draft")
async def api_refine_draft(contact_id: int, current_draft: str, tone: str = "Recommended"):
    import openai
    tone_mode = tone if tone in TONE_PROFILES else "Recommended"
    prompt = f"Refine and slightly tighten the following outreach draft without changing the tone or meaning. Keep <= 90 words. Tone style: {TONE_PROFILES.get(tone_mode, '')}\n\n---\n{current_draft}\n---"
    try:
        resp = openai.ChatCompletion.create(model="gpt-4-turbo", messages=[{"role":"user","content":prompt}], temperature=0.3)
